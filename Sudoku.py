...
class SudokuGUI(Frame):
    ...
    def __init__(self, master):
        ...
        # generate new game
        self.ng = Button(self.bframe, text='Tạo game mới', width=20, command=self.new_game).pack(anchor=S)
        # solver
        self.sg = Button(self.bframe, text='Giải', width=20, command=self.solver).pack(anchor=S)
    ...
    def solver(self):
        ...
        if (solution):
            if generation == -1:
                ...
                str_print = "Đầu vào không hợp lệ. Sinh lại đề bài mới!"
            elif generation == -2:
                print("Không tìm được lời giải")
                str_print = "Không tìm được lời giải, Vui lòng thử lại!"
            else:
                self.grid_2 = solution.values
                self.sync_board_and_canvas_2()
                time_elapsed = '{0:6.2f}'.format(time.time()-start_time)
                str_print = "Lời giải tìm được ở lần lai: " + str(generation) + \
                        "\n" + "Thời gian : " + str(time_elapsed) + "s"
